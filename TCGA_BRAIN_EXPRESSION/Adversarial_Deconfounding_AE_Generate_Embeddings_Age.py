...
#Read age labels
filename = "TCGA_GBM_and_LGG_AGE_LABELS.tsv"
...
scale_df = lambda df, scaler: pd.DataFrame(scaler.transform(df), columns=df.columns, index=df.index)
Z_train = Z_train.pipe(scale_df, scaler) 
Z_test = Z_test.pipe(scale_df, scaler)
...
class AdversarialDeconfoundingAutoencoder(object):
    ...
    def fit(self, x, z, validation_data=None, T_iter=250, batch_size=128):
        ...
        for idx in range(T_iter):
            ...
            self._train_metrics.loc[idx, 'Adversary mse'] = history.history['mse'][0]
            self._val_metrics.loc[idx, 'Adversary mse'] = history.history['val_mse'][0]
            ...
            self._train_metrics.loc[idx, 'Adversary MSE'] = history.history['adversary_mse'][0]
            self._val_metrics.loc[idx, 'Adversary MSE'] = history.history['val_adversary_mse'][0]
        ...
        plt.plot(self._train_metrics['Adversary MSE'], 
                 label = 'Adversary training MSE', lw = 5, color = '#16a085')
        plt.plot(self._val_metrics['Adversary MSE'], 
                 label = 'Adversary validation MSE', lw = 5, color = '#9b59b6')
...
embedding_df.to_csv('ADV_FILES_AGE/ADV_Embedding_' + str(latent_dim) + 'L_lam' + str(lambda_val) + '_fold' + str(run) + '_5k_epochs.tsv')

#Record models
model_json = adv_model._encoder.to_json()
with open('ADV_FILES_AGE/ADV_encoder_' + str(latent_dim) + 'L_lam'+ str(lambda_val) +'_fold'+ str(run) +'_5k_epochs.json', "w") as json_file:
    json_file.write(model_json)
adv_model._encoder.save_weights('ADV_FILES_AGE/ADV_encoder_' + str(latent_dim) + 'L_lam'+ str(lambda_val) +'_fold'+ str(run) +'_5k_epochs.h5')
print("Saved model to disk")

model_json = adv_model._decoder.to_json()
with open('ADV_FILES_AGE/ADV_decoder_' + str(latent_dim) + 'L_lam'+ str(lambda_val) +'_fold'+ str(run) +'_5k_epochs.json', "w") as json_file:
    json_file.write(model_json)
adv_model._decoder.save_weights('ADV_FILES_AGE/ADV_decoder_' + str(latent_dim) + 'L_lam'+ str(lambda_val) +'_fold'+ str(run) +'_5k_epochs.h5')
print("Saved model to disk")
